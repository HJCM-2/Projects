...
for i in range(episode):
    ...
    for j in range(steps):
        ...
        if terminal == 1:
            ...
            # print('steps:', j)
            # print('finish_episode:', finish_episode)
            # print('total_steps:', total_steps)

            break
        # For the case that is not a terminal state, target_Q is R+γ max_a′ Q(S′, a′)
        agent.Q[s, a] = agent.Q[s, a] + alpha * (r + gamma * (np.max(agent.Q[s1, :])) - agent.Q[s, a])
    ...
    if i % 20 == 0 and i != 0:
        ...
        plt.savefig('./Target_rate_10_qlearn.jpg')
        plt.draw()
        # time.sleep(5)
        plt.figure(2)
        ...
        plt.savefig('./Average_step_10_qlearn.jpg')
        plt.draw()
        # time.sleep(5)
