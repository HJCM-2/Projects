...
def train(sess, env, network, reward, discrete, q, q1, l):  
    # Set up summary writer
    summary_writer = tf.summary.FileWriter("dqn_summary_test")
    ...
    for i in range(MAX_EPISODES):
        ...
        while not terminal and not rospy.is_shutdown():
            ...
            env.Control(action2)
            fl1 = fl1 + (action2[0]-action3[0])**2
            fl2 = fl2 + (action2[1]-action3[1])**2
            ...
            if i % 1 == 0:
                ...
                np.save("./data_test/speed_v%s.npy" % (i), c)
                np.save("./data_test/speed_w%s.npy" % (i), d)
        ...
        fl1_avg = fl1 / j
        fl2_avg = fl2 / j
        fl1_total_avg = fl1_total_avg + fl1_avg
        fl2_total_avg = fl2_total_avg + fl2_avg  
        print 'fluctuation_v: ', fl1_avg, 'fluctuation_w: ', fl2_avg
        print ('| Reward: %.2f' % ep_reward, " | Episode:", i, " | LoopTime: %.4f" % (np.mean(loop_time_buf)), " | Step:", j-1, '\n')

    print 'reach: ', fr, float(fr)/float(MAX_EPISODES) , 'crash: ', fc, float(fc)/float(MAX_EPISODES) , 'time out: ', ft, float(ft)/float(MAX_EPISODES)
    print 'total_fluctuation_v', fl1_total_avg/MAX_EPISODES, 'total_fluctuation_w', fl2_total_avg/MAX_EPISODES
    q.insert(l, [fl1_total_avg/MAX_EPISODES, fl2_total_avg/MAX_EPISODES])
    q1.insert(l, [fr, float(fr)/float(MAX_EPISODES), fc, float(fc)/float(MAX_EPISODES), ft, float(ft)/float(MAX_EPISODES)])
    return q, q1
...
def main(_):
    with tf.Session() as sess:
        ...
        for l in range(0, TIMES):
            try:
                q, q1 = train(sess, env, network, reward, discrete, q, q1, l)
                print q
                print q1
                print '--------------------------------', l, 'is over ---------------------------------'
